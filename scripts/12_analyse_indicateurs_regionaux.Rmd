...
reg_indicateur_graph_reduit <- reg_indicateur_lrr_tendances_lm %>%
  ...
  filter(!(esp_code_alternatif %in% c("ANG", "SAT", "GAR","TRF"))) %>% 
  ggplot(aes(x = annee, 
             y = valeur, 
             group = stade, 
  ...
  geom_vline(data = . %>% filter(esp_code_alternatif %in% c("CHA", "GOU") & annee == 2013), 
  ...
  geom_vline(data = . %>% filter(esp_code_alternatif %in% c("PER", "VAI", "VAR") & annee == 2011), 
...
On souhaite observer la significativité avec un test de régression linéaire simple : 
...
# On souhaite faire la même chose avec les test de mann-kendall qsuivis de la regression de Sen-Theil : 
