...
POINT EXPERIMENTATION GLM ET LM : 
...
#Mise en place du dataframe original avec les protocoles et les réseaux



m_espece <- "GAR"
m_stade <- "ind"
m_indicateur <- "densite_surface"


# Petit jeu de données
ope_reduc <- ope_indicateur %>% 
  rename(espece = esp_code_alternatif) %>%
  filter(espece%in%c("ANG","GAR"),
         indicateur %in% c("biomasse","densite_surface")) %>%
...
# Fonction pour calculer le modèle pour une combinaison donnée d'espèce, stade et indicateur
calculate_model_glmm <- function(data, 
  ...
  model <- lmer(valeur ~ (1 | pop_id) + annee + pro_libelle + obj_libelle, data = filtered_data)
...
# Appliquer cette fonction pour chaque combinaison d'espèce, stade et indicateur
...
result_final <- application_modeles(ope_indicateur_glm)
...
application_modeles <- function(data) {
  ...
  results_list <- combinations %>%
    mutate(result = pmap(.l = combinations,
                         .f = ~ calculate_model_glmm(data, ..1, ..2, ..3)))
  return(results_list)
}
