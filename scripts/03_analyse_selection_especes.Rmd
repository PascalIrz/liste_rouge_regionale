---
title: "Liste Rouge Régionale des poissons d'eau douce de Bretagne"
subtitle: "Sélection des espèces"
author: "OFB Bretagne"
date: "`r format(Sys.time(), '%Y-%m-%d')`"
params:
  annee_limite: 1990 # On fixe une année limite afin de calculer les densités
  especes_lrr_2015: !r c("ABL","ALF","ANG","BRB","BRE","BRO","CCO","CHA","CHE",
                         "EPI","EPT","FLE","GAR","GOU","ALA","LPP","LPR","LPM",
                         "LOF","MUP","PER","ROT","SAT","TAN","TRF","VAI","VAR", 
                         "GRE")
  especes_a_retirer: !r c("APP","ASL","BRX","CAX","CYP","HBG","OCL","PCC","PFL",
                       "CCU","CAS") # Sélection des espèces à retirer du jeu de données initial
output:
  bookdown::html_document2:
    toc: true
    toc_float: true
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE,
                      message = FALSE,
                      warning = FALSE)
```

# Objectif : Dynamique de population des poissons d'eau douce de Bretagne - 03_analyse_selection_especes

Pour les opérations sélectionnées, on cherche à savoir quelles espèces seront sélectionnées pour les futures analyses. On souhaite réaliser un graphique bi_varié et de sélectionner les espèces à partir d'un seuil déterminé. 

Donc : on réalise une sélection non pas sur les espèces appartenant à la liste rouge régionale de 2015 mais à partir de seuils. Pour chaque espèce on fait un graphique bivarié : on met pour chaque espèce le nombre de fois où elle a été observée et sa densité moyenne sur la période des 10 dernières années. A partir de ce graphique on élimine les espèces qui sont trop rares, pour lesquelles nous n'avons pas assez de données. Les espèces qui sont trop peu abondantes sont aussi abandonnées. On fixe donc un seuil qui nous donnera une liste d'espèce. 

# Installation 

## Chargement des packages et des données 

```{r}
## Chargement des packages ----

library(tidyverse)
library(aspe)
library(ggplot2)
library(zoo)
library(lemon)

## Chargement des données ----
load(file = "../processed_data/selection_pop_ope.rda")

rdata_tables <- misc_nom_dernier_fichier(
  repertoire = "../../../../projets/ASPE/raw_data/rdata",
  pattern = "^tables")
load(rdata_tables)

mei_table <- misc_nom_dernier_fichier(
  repertoire = "../../../../projets/ASPE/raw_data/rdata",
  pattern = "^mei")
load(mei_table)

```

# Constitution du jeu de données

## Création d'un df contenant les données d'opérations et d'effectifs des années sélectionnées :

```{r}
#Je réalise un df contenant toutes les espèces présentes dans mes opérations de pêches
lop_esp_selection <- passerelle %>% 
  left_join(y=lot_poissons %>% 
              select(lop_id,
                     esp_id = lop_esp_id,
                     lop_effectif)) %>% 
  left_join (y= ref_espece %>% 
               select(esp_id,
                      esp_code_alternatif)) %>% 
  left_join (y=operation %>% 
               select (ope_id, 
                       ope_surface_calculee,
                       passage$pas_numero))


lop_esp_selection <- lop_esp_selection %>% 
  mef_ajouter_ope_date() %>% 
  select(ope_id,
         lop_effectif,
         lop_id,
         esp_code_alternatif,
         annee,
         ope_surface_calculee) %>% 
  distinct()


# Je remplace les VAN par les VAR (vandoises Rostrées)
lop_esp_selection <- lop_esp_selection %>% 
  mutate(esp_code_alternatif = ifelse (test = esp_code_alternatif == "VAN",
                                       yes = "VAR",
                                       no = esp_code_alternatif)) %>% 
  filter(!(esp_code_alternatif %in% params$especes_a_retirer))

# On réduit le jeu de donnée à notre année limite (paramètre)
lop_esp_selection_calcul <- filter(lop_esp_selection, annee >= params$annee_limite)

```

On calcul notre densité moyenne d'espèce sur les 30 dernières années :

```{r}

densite_moyenne <- lop_esp_selection_calcul %>% 
  group_by(ope_id,
           esp_code_alternatif,
           ope_surface_calculee) %>% 
  summarise(effectif= sum(lop_effectif)) %>% 
  ungroup() %>% 
  mutate(indicateur= "densite_surface") %>% 
  mutate(valeur = effectif/ope_surface_calculee)

# Calcul de l'effectif total pour chaque espèce
densite_moyenne <- densite_moyenne %>%
  group_by(esp_code_alternatif) %>%
  mutate(effectif_total = sum(effectif))

# Calcul de la densité moyenne pour chaque espèce
densite_moyenne <- densite_moyenne %>%
  group_by(esp_code_alternatif) %>%
  mutate(densite_moyenne = mean(valeur))
```

On réalise le calcul du pourcentage d'occurrence qui se trouvera en ordonnée : 

```{r}
ope_id <- unique(lop_esp_selection_calcul$ope_id)
esp_code_alternatif <- unique(lop_esp_selection_calcul$esp_code_alternatif)

annees <- lop_esp_selection_calcul %>% 
  ungroup() %>% 
  select(ope_id,
         annee) %>% 
  distinct()

tab_oc <- crossing(ope_id, esp_code_alternatif) 

#Calcul du pourcentage d'occurrence (nombre de pêches avec présence / nombre de pêches)
taux_occurrence <- tab_oc %>% 
  left_join(densite_moyenne) %>% 
  mutate(valeur_oc = ifelse(is.na(valeur), 0, valeur)) %>% 
  left_join(annees) %>% 
  group_by(esp_code_alternatif,
           annee) %>% 
  summarise(n_ope = n_distinct(ope_id),
            n_oc = n_distinct(ope_id[valeur_oc > 0]),
            taux_oc = (n_oc / n_ope)*100) %>% 
  group_by(esp_code_alternatif) %>% 
  mutate(moy_taux_oc = mean(taux_oc))


donnees_graphique <- left_join(densite_moyenne, taux_occurrence, by = "esp_code_alternatif")

donnees_graphique <- donnees_graphique %>%
  mutate(couleur = ifelse (esp_code_alternatif %in% params$especes_lrr_2015, 
                           "Liste Rouge Régionale Bretagne 2015",
                           "Présentes en Bretagne"))

```


# Représentation graphique des résultats 

```{r, fig.height = 10, fig.width = 10}

# Visualisation du graphique avec ggplot2
ggplot(
  donnees_graphique,
  aes(
    x = densite_moyenne,
    y = moy_taux_oc,
    label = esp_code_alternatif,
    color = couleur
  )) +
  geom_point(size = 3,
             shape = 16) +
  geom_text(
    hjust = -0.4,
    vjust = 1.3,
    size = 3,
    fontface = "bold",
    color = "black"
 ) +
  labs(
    title = "Densité moyenne vs Pourcentage d'occurrence par espèce",
    subtitle = "Région Bretagne",
    x = "Densité moyenne sur les 30 dernières années",
    y = "Pourcentage d'occurrence") +
  scale_color_manual(values = wesanderson::wes_palette("AsteroidCity1")[3:5],
                     name = "Espèces") +
  scale_x_log10() +  # axe des x en échelle logarithmique
  scale_y_log10() +  # axe des y en échelle logarithmique
  theme_light(base_size = 11) +
  theme(
    plot.title = element_text(hjust = 0.5),
    panel.grid.major = element_line(color = "#ffffff", size = 0.1),
    panel.grid.minor = element_line(color = "#ffffff"),
    panel.background = element_rect(fill = "#faf0e6"),
    legend.position = "bottom") 
```

# Sauvegarde : 
```{r}
save(lop_esp_selection,
     file = "../processed_data/analyse_selection_especes.rda")
```

