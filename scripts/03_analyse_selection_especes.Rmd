...
#Les espèces de la liste rouge régionale 2015
...
donnees_graphique <- donnees_graphique %>%
  mutate(couleur = ifelse(densite_moyenne >= seuil, "Densité >= 0.01", "Densité < 0.01"))
```
...
ggplot(donnees_graphique, aes(x = densite_moyenne, 
                              y = moy_taux_oc, label = esp_code_alternatif, 
                              color = couleur)) +
  geom_point(size = 3, 
             shape = 16) +
  geom_point(data = subset(donnees_graphique, esp_code_alternatif %in% vecteur1), 
                     color = "red", 
                     size = 5, 
                     shape = 1) +
  geom_text(hjust = -0.4, 
