...
```{r}
load(file = "../processed_data/mk_lmm_lm_tendances.rda")
...
df_combined <- reg_indicateur_lrr_tendances_lm %>%
  ...
  rename(trend_lm = trend, 
         sig_lm = sig) %>%
  left_join(reg_indicateur_lrr_tendances_lmm %>% 
              select(esp_code_alternatif,
                     indicateur,
                     trend,
                     sig) %>% 
              rename(trend_lmm = trend, sig_lmm = sig), 
...
```{r, fig.height = 8, fig.width = 10}
colnames(df_final1)[1:11] <- c("Code espèce", "Période", "Indicateur", "Tendance", "Sig.", "Tendance", "Sig.", "Tendance", "Sig.", "Tendance", "Sig.")

d <- kable(df_final1,
      booktabs = TRUE,
      escape = FALSE,
      align = c("l", "l", "l", "c", "l", "c", "l", "c", "l", "c", "l")) %>%
  add_header_above(c(" " = 3, "Modèle lm" = 2,  "Modèle mk_st" = 2, "Modèle lmm" = 2, "Modèle bayésien"= 2)) %>%
