...
Les pré-traitements nécessaires à la consitution de ce tableau de bord ont été réalisés dans les scripts précédents. Ce script permet donc de constituer le tableau de bord comportant plusieurs indicateurs dits "simples". Ces indicateurs (densités volumiques, de surface, pourcentage de juvéniles, longueurs médianes,...) seront calculés à l'échelle des opérations de pêches retenues pour l'étude. Ces indicateurs sont calculés par espèces et par stade (juvéniles / adultes), mais également pour l'ensemble des stades (indeterminés). 
...
NB : Nous considerons les eventuels NA en *mei_id* précédemment repérés dans le script 04_pre_traitements_donnees_ope_especes (et leurs affectons les données d'effectifs du lop_id correspondant).
...
```{r Creation tableau effectif total par ope et stade}
...
Nous avons donc une première version des effectifs sans avoir inclut les absences. Les absences des espèces sont lorsque une espèce a déjà été dans une station / point de prélèvement lors d'une années mais qu'elle n'y a pas été retrouvée lors des années suivantes. 

# 1. Créer un dataframe avec toutes les combinaisons possibles d'ope_id, esp_code_alternatif et années présentes dans vos données
...
### Densité surfacique par opération de pêches
...
La fonction *calcul_densite_surface* calcule les densités de surfaces par opérations / espèces et stades : les résultats de cette fonction sont renvoyés dans le jeu de données *ope_densite_surf*. 
...
### Densité volumique par opération de pêche
...
### Pourcentage de juvéniles par opération de pêches
...
### Ecart interquartile des longueurs médianes
...
Nous sauvegardons à part les données d'effectifs et de densités de surfaces qui nous servirons ultérieurement : 
