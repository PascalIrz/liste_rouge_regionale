...
## Création de la carte de situation
  ...
  Dans un premier temps, il s'agit de créer une petite carte de situation permettant de situer l'aire géographique étudiée à l'échelle de la France (ici la Bretagne). 

```{r Création carte situation}
france <- ne_countries(country = "france", returnclass = "sf") # Sélection du pays de l'étude (#France)

carte_france_dept <- ggplot(data = france) + # Création de la zone précise de l'étude (#Bretagne)
...
## Création de la carte station
  ...
  Il s'agit de créer la carte principale regroupant les stations étudiées (données issues de pré-traitements réalisés dans le script *00_selection_pop_ope*). 
...
logo <- readPNG("../assets/ofb_logo.png") # Ajout du logo de l'OFB
...
titre <- ggdraw() + # Ajouter un titre
  draw_label("Stations bretonnes sélectionnées pour 
             l'appui à l'actualisation de la Liste Rouge Régionale des poissons d'eau douce 2024", fontface = 'bold', x = 0.5, y = 0.70, size = 10) + 
