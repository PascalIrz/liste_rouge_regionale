...
subtitle: "Analyses bayésiennes"
author: "OFB Bretagne"
date: "`r format(Sys.time(), '%Y-%m-%d')`"
params:
  mes_especes: !r c("ANG", "BRO", "LOF", "CHE", "CHA", "GAR", "GOU", "VAI", "PER","TRF", "LPP", "SAT", "VAR")
output:
...
Installation des packages nécessaires au fonctionnement du script : 
...
library(aspe) # Chargement des packages 
library(aspe2)
library(nimble)
library(popdynmodel)
library(dplyr)
...
## Nommage des fichiers enregistrés
...
Cette partie nécessite le chargement des packages *popdynmodel* et *nimble*. Nous utilisons la fonction *mod_popgrow* pour estimer la croissance de la population des espèces à tester pour la liste rouge régionale sur la période 1990-2023, de l’ajustement du modèle de croissance à l’ensemble de données sur les espèces. Comme nous souhaitons tenir compte des variations annuelles de l’effort d’échantillonnage (ici la surface de pêche), nous devons spécifier l’argument *var_surf*. Nous souhaitons également les estimations de croissance démographique sur périodes spécifiques que nous indiquons en variables de la fonction.
...
Nous appliquons donc la fonction sur notre jeu de données *data_bayesien_lrr* : 

```{r Application modèle bayésien, eval= FALSE}
data <- data %>% 
...
mod_popgrow_result <- mod_popgrow(data,
                  var_id = "pop_id",
                  var_tmp = "annee",
...
La fonction *mod_popgrow()* retourne une liste avec les résumés des principales statistiques associées à l’estimation des paramètres demandés (mcmc_summary), les chaînes MCMC (mcmc_chain) et la liste des indices de paramètres (subscript). Nous sauvegardons ces résultats :

```{r Sauvegarde modèle bayésien, eval= FALSE}
save(mod_popgrow_result,
     file = "../processed_data/bay_final_result_pro.rda")
```


# Analyse des résultats et vérification de la convergence du modèle

```{r Vérification convergence modèle, eval= FALSE}
mod_popgrow_result$mcmc_summary
denplot(mod_popgrow_result$mcmc_chain)
plot(mod_popgrow_result$mcmc_chain$chain2)
...
```{r Représentation graphique, eval= FALSE}
denplot(mod_popgrow_result$mcmc_chain, collapse = TRUE)

temp <- mod_popgrow_result[["mcmc_summary"]]
temp %>% 
  filter(is.na(pop_id)) %>% 
  nrow
  Sys.time()

traplot(mod_popgrow_result$mcmc_chain, parms = c("p.col_id[1, 0 ]"))      
```
