...
On souhaite considérer l'effet du protocole : 

```{r}

mes_especes <- c("ANG", "BRO", "LOF","CHE", "CHA", "GAR", "GOU", "VAI", "PER","TRF", "LPP", "SAT", "VAR")
data_pro <- data %>% 
...
```{r}
mc_pro <- mod_popgrowAlt(data_pro,
                  var_id = "pop_id",
                  var_tmp = "annee",
                  var_tax = "esp_code_alternatif",
                  var_cnt = "catch_size",
...
save(mc_pro,
     file = "../processed_data/bay_final_result2.rda")
...
```{r,  fig.height = 145, fig.width = 140}
mc$mcmc_summary

denplot(mc$mcmc_chain)

plot(mc$mcmc_chain$chain2)
