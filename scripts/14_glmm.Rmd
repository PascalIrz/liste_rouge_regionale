...
Je fait un jeu de données pour une seule espèce (avant de créer une fonction qui le fera pour tout le monde)

```{r Construction jeu de donnees 2}
ope_glm_complet <- ope_glm_complet %>%
  filter(
    (esp_code_alternatif == "ANG" & annee >= 1990) |
    (esp_code_alternatif %in% c("CHE", "LPP") & annee >= 2005) |
    (esp_code_alternatif %in% c("BRO", "LOF") & annee >= 2008) |
    (esp_code_alternatif %in% c("VAR", "GAR", "VAI", "PER") & annee >= 2011) |
...
```{r Construction jeu de donnees 2}
ope_glm_esp <- ope_glm_complet %>%
  filter(esp_code_alternatif == "VAI") %>% 
  mutate(pop_id = as.factor(pop_id)) %>% 
  rename(espece = esp_code_alternatif)

ope_glm_esp$X <- 1:nrow(ope_glm_esp)
```
...
```{r}
mod2 <- glmer(valeur ~ annee + scale(ope_surface_calculee) + (1| pop_id) +
              #(1|X) +
               pro_libelle, 
...
```{r}
ope_glm_esp$predicted_valeur <- predict(mod2, 
                                     newdata = ope_glm_esp, 
                                     type = "response", 
                                     re.form = NA)
```
...
ope_glm_complet1 <- ope_glm_complet %>% 
  rename(espece = esp_code_alternatif) %>% 
  ungroup() %>% 
  select(-ope_id,
         -indicateur,
...
```{r Excecution fonctions GLM}
glmm_resultats <- glmm_application_modele(data = ope_glm_complet1,
