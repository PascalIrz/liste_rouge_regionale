...
# Point de vue global sur les données disponibles : 

```{r}
# Filtrer les données pour l'indicateur 'effectif_total'
pnt_vue <- ope_indicateur %>%
...
## Dataframe ope_selection : On souhaite y ajouter les 0 aux valeurs manquantes, on réalise la même opération : 
...
On observe l'allure de l'occurrence des espèces par station (les espèces au stade adulte) : 
...
```{r}
nombre_total_stations <- n_distinct(occur_sta_esp$pop_id)
nombre_valeurs_esp <- length(unique(occur_sta_esp$esp_code_alternatif))
nombre_total_annee <- 33 # pas bien il faudra que je modifie
...
tableau_final <- occur_sta_esp %>%
  group_by(esp_code_alternatif, pop_id) %>%
  summarize(somme_obs_esp_sta = sum(valeur > 0)) %>% 
  ungroup()


# Filtrer les espèces présentes sur plus de 25% au sein de chacune des stations
tableau_final <- tableau_final %>%
  group_by(esp_code_alternatif) %>%
  filter(somme_obs_esp_sta >= 0.25 * nombre_total_annee) %>% 
  mutate(nombre_total_stations_25 = n_distinct(pop_id))

# Afficher le tableau final filtré
print(tableau_final)

# J'ai donc produit un tableau où j'ai mes espèces et mes stations qui sont intéressantes à regarder ! 
...
Réalisation d'un tableau bilan des espèces à traiter avec leurs temps de génération et temps d'études associés : 
...
colnames(reg_espece_tab) <- c("Nom commun", "Nom de code", "Durée d'étude LRR","Temps de génération")
