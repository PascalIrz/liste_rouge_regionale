...
```{r}
mes_sta_id_ang <- c("10502","10546","10844","10950","11224","11290","11432","11539",
                    "11593","11601","11602","11735","11605","11743","12007","12060", 
                    "12118","12291", "12316","12325","12371","12402","12436")


mon_espece_lrr <- c("ANG")
...
data <- mef_creer_passerelle () %>%
  ...
  filter(sta_id == mes_sta_id_ang,
         annee %in% 1990:2023)
  # filter(annee %in% 1990:2023, 
  #        pop_bas_id == 4, 
  #        esp_code_alternatif == mon_espece_lrr,
  #        sta_id == mes_sta_id_ang) 

data$pas_numero[is.na(data$pas_numero)] <- 1
...
data_ang <- filter(data, esp_code_alternatif %in% "ANG") %>%
...
```{r}
result_29_05 <- mod_popgrow(data_ang,
                      var_id = "pop_id",
...
save(result_29_05,
     file = "../processed_data/modelisation_croissance_demographique_result_29_05_ang.rda")
```
...
traplot(result$mcmc_chain, parms = c("p.col_id[2, 10]"))  
...
save(result_popgrowAlt,
     file = "../processed_data/modelisation_croissance_demographique_result_popgrowAlt_22_05.rda")
...
```{r}
result_trf <- mod_popgrow(data_trf,
                      var_id = "pop_id",
                      var_tax = "esp_code_alternatif",
                      var_tmp = "annee",
                      var_cnt = "catch_size",
