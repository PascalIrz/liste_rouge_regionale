...
```{r}
data_filter <- mef_ajouter_lots(data) %>%
...
```{r}
data_filter <- mef_ajouter_absence(data_filter,
                            var_id = pop_id,
...
```{r}
data_lrr <- filter(data_filter, 
                   esp_code_alternatif %in% mon_espece_lrr) %>%
...
```{r}
result_27_05_ang <- mod_popgrow(data_lrr,
                      var_id = "pop_id",
                      var_tax = "esp_code_alternatif",
                      var_tmp = "annee",
                      var_cnt = "catch_size",
                      var_surf = "ope_surface_calculee",
                      period = list(c(2015,2023)),
                      n_iter = 1005000,
                      n_thin = 100,
                      n_burnin = 5000)
