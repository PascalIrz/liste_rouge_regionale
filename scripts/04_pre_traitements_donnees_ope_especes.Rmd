...
#   filter(!pas_numero%in%params$passage_a_retirer) %>%
#   distinct() %>% # Retirer les éventuels doublons
...
On observe dans le jeu de données des potentielles valeurs absentes / manquantes. Ceci peut être dû à des problèmes de retranscription dans la base de données ASPE. Parfois on observe une absence de marquage de valeurs individuelles. Je les recueilles dans un tableau : 

```{r}
# valeur_na <- mei_esp_ope_selection %>% 
#   filter(if_any(everything(), is.na))
# 
# mei_esp_ope_selection <- mei_esp_ope_selection %>% 
#   na.omit(mei_esp_ope_selection)
```

Je souhaite maintenant, pour la suite, y mettre des valeurs de tailles indicatrices moyennes selon l'opération dans lesquelles elles se trouvent. Je souhaite également indiquer la station à partir de l'opération. 
...
Je réalise un tableau de référence de taille pour pouvoir donner des valeurs de tailles indicatives là où il en manque. 

```{r}
# tableau_reference <- mei_esp_ope_selection %>%
#   group_by(esp_code_alternatif) %>%
#   summarise(mei_n = median(mei_taille, na.rm = TRUE)) %>%
#   ungroup()

```

Je vais donc remplacer les valeurs de tailles manquantes par les valeurs du tableau de référence

```{r}
# tab_valeur_complet <- valeur_na %>% 
#   group_by(ope_id, esp_code_alternatif) %>% 
#   left_join(tableau_reference) %>% 
#   select (ope_id,
#           lop_effectif,
#           lop_id,
#           esp_code_alternatif,
...
#           espece_presence_lrr_2015,
#           taille_maturite) %>% 
#   rename (mei_taille = mei_n)
```

On ajoute les valeurs maintenant ajoutée 

```{r}
# mei_esp_ope_selection <- bind_rows(mei_esp_ope_selection, tab_valeur_complet)
```
