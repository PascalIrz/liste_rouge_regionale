...
Etape 1 : Identification des valeurs aberrantes et très aberrantes :
...
donnees_env_aberrantes <- ggplot(ope_selection_param_env, aes(x = annee, y = valeur, fill = aberrant_color)) +
  ...
  scale_fill_manual(values = c("Normal" = "#2C7BB6", "Aberrant" = "orange", "Très aberrant" = "red")) +
...
mutate(valeur = ifelse(very_aberrant,
                         ...
                         valeur)) %>%
  ungroup() %>%
  select(-Q1, -Q3, -IQR, -lower_bound, -upper_bound, -median_val)
```
