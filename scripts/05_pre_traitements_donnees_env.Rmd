...
ope_selection_param_env <- ope_selection_param_env %>% 
  arrange(parametre, annee)%>%  # Trier les données environnementales globales
  group_by(parametre,pop_id) %>%
  mutate(valeur = ifelse(is.na(valeur), # Remplacer les NA par les médianes
