  ...
  
{
  ...
  # Construction d'un df des 75 percentiles des tailles des différents statuts des espèces (juvéniles / adultes) ----
  ope_75_percentile_statut <- df %>%
    group_by(!!var_id_site,
             !!var_esp_code_alternatif,
             !!var_statut) %>% 
    summarise(valeur=quantile(!!var_taille, probs = c(.75), na.rm=TRUE))
  
  ope_75_percentile_statut <- ope_75_percentile_statut %>% 
  ...
  ope_75_percentile_esp <- ope_75_percentile_esp %>% 
    mutate(indicateur="75_percentile", statut = "toutes") %>% 
    select(!!var_id_site,
           !!var_esp_code_alternatif,
           indicateur,
           valeur,
  ...
  # Construction d'un Df avec les 75 percentiles des espèces par opération toutes tailles confondues + des différents statuts ----
  ope_75_percentile <- bind_rows(ope_75_percentile_statut, ope_75_percentile_esp)
  
  list(df1 = ope_75_percentile, 
       df2 = ope_75_percentile_esp,
       df3 = ope_75_percentile_statut)
} 